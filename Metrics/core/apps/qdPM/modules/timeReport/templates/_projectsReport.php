<div class="pageHeading">
  <table>
    <tr>
      <td><span class="pageHeading"><?php echo __('Cases') ?></span></td>
      <td style="padding-left: 15px;"><?php echo __('Export') .': <a href="#" onClick="time_report_export(\'projects_export\',\'csv\');">.csv</a> | <a href="#" onClick="time_report_export(\'projects_export\', \'txt\');">.txt</a>' ?></td>          
    </tr>  
  </table>
</div>
...
<?php
  ...
  $count_items = 0;
?>
  <table class="tableListing" id="tableListingProjects">
    <thead>
      <tr>    
    ...
    ?>
     <tr onClick="view_project_time_report(<?php echo $project->getId(); ?>)" style="cursor: pointer;">
       <td><?php echo link_to($project->getName(),'timeReport/' . $sf_context->getActionName(),array('title'=>__('Project Info'),'class'=>'jt','rel'=>url_for('projects/info?id=' . $project->getId()),'query_string'=>'filter_by[Projects]=' . $project->getId())) ?></td>
...
<form action="<?php echo url_for('timeReport/export')?>" method="post" id="projects_export">
  <?php echo input_hidden_tag('filename','projects').input_hidden_tag('format').input_hidden_tag('export',serialize($export)) ?>
</form>     
   
<script type="text/javascript">
  $(document).ready(function(){  
    $("#tableListingProjects").tablesorter({widgets: ['zebra']})   
  });     
</script>
