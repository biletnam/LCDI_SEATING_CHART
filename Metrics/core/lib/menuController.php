...
class menuController
{
  ...
  public function __construct($sf_user,$sf_request)
    ...
    foreach($reports as $r)
    {
	  ...
	  {	                 
      $s[] = array('title'=>__('Add Case'),'url'=>'projects/new','modalbox'=>true,'is_hr'=>$is_hr); //'Add Project' to 'Add Case'
      $is_hr = false;
    }

    $s[] = array('title'=>__('View All'),'url'=>'projects/index','is_hr'=>$is_hr);
	  
    return array('title'=>__('Cases'),'url'=>'projects/index','submenu'=>$s); //'Projects' to 'Cases'
	}
    ...
    return array('title'=>__('Tasks'),'url'=>'tasks/index','submenu'=>$s);*/ //Commented out 03/24
	}
    ...
    $s = array();
	  ...
	  {	                 
      $s[] = array('title'=>__('Add Client'),'url'=>'tickets/new','modalbox'=>true);
      $is_hr = false;
    }

    $s[] = array('title'=>__('View All'),'url'=>'tickets/index','is_hr'=>$is_hr);
	
    return array('title'=>__('Clients'),'url'=>'tickets/index','submenu'=>$s); //Commented out 03/24
	}
    ...
    return array('title'=>__('Discussions'),'url'=>'discussions/index','submenu'=>$s);*/ //Commented out 03/24
	}
  ...
  /*ZOMG dont touch this it breaks all the things pls k thanx


  */
  public function buildClientsMenu()
  {
    $s = array();

    //$rm = $this->buildReportsMenuByTable('Clients');

    $s[] = array('title'=>__('Add Client'), 'url'=>'tickets/new');
    $s[] = array('title'=>__('View All'), 'url'=>'tickets/index');//,'submenu'=>(count($rm)>0?$rm:null));
    
    if(count($s)>0)
    {
      return array('title'=>__('Clients'),'submenu'=>$s);
    }
    else
    {
      return false;
	...
	public function buildReportsMenu()
	{
      ...
      $s[] = array('title'=>__('Case Reports'),'url'=>'projectsReports/index','submenu'=>(count($rm)>0?$rm:null)); //'Project Reports' to 'Case Reports'
    ...
    return array('title'=>__('Configuration'),
                 'submenu'=>array(
                    ...
                    array('title'=>__('Cases'), //Formerly 'Projects'
                      'submenu'=>array(
