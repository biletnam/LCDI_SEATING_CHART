...
class projectsMenuController
{
  ...
  public function __construct($sf_user,$sf_request)
	{
	  ...
	  //$this->clients_id = $sf_request->getParameter('clients_id);

    $this->access = array();        
	  $this->access['projects'] = Users::getAccessSchema('projects',$sf_user);
	  //$this->access['clients'] = Users::getAccessSchema('clients', $sf_user);
	...
	public function buildMenu($sf_context)
  {
    ...
    {
      $m[] = array('title'=>__('Case Info'),'url'=>'projectsComments/index?projects_id=' . $this->projects_id,'is_selected'=>($sf_context->getModuleName()=='projectsComments'));
    }

    /*if($this->access['clients']['view'] or $this->access['clients']['view_own'])
    {
	$s = array();

	if($this->access['clients']['insert'])
	{
		$s[] = array('title'=>__('Add Client'),'url'=>'clients/new?clients_id='. $this->clients_id,'modalbox'=>true);
	}

	$s[] = array('title'=>__('View All'),'url'=>'clients/index?clients_id='. $this->clients_id);

	$m[] = array('title'=>__('Client Info'),'url'=>'clientsComments/index?clients_id=' . $this->clients_id,'is_selected'=>($sf_context->getModuleName()=='clientsComments'));
    }*/
    ...
    if($this->access['tickets']['view'] or $this->access['tickets']['view_own']) 
    {
      ...
      if($this->access['tickets']['insert'])
  	  {	                 
        $s[] = array('title'=>__('Add Client'),'url'=>'tickets/new?projects_id='. $this->projects_id,'modalbox'=>true);        
      }
      $s[] = array('title'=>__('View All'),'url'=>'tickets/index?projects_id='. $this->projects_id);
      
      $m[] = array('title'=>__('Clients'),'url'=>'tickets/index?projects_id=' . $this->projects_id,'submenu'=>$s,'is_selected'=>in_array($sf_context->getModuleName(),array('tickets','ticketsComments')));
    }
