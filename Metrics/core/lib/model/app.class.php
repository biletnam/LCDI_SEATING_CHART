...
class app
{
  ...
  public static function getArrayNameWithIcon($o,$k)
  {
    if(isset($o[$k]))
    {
      ...
      {
        return '<span class="hiddenItem">' . $o[$k]['sort_order'] . '_' . $o[$k]['name'] . '</span>' . image_tag('icons/p/'.$o[$k]['icon'],array('title'=>$o[$k]['name']));
      }
      else
      {
        return '<span class="hiddenItem">' . $o[$k]['sort_order'] . '_' . $o[$k]['name'] . '</span>' . $o[$k]['name'];
      }
    }
    else
    {
    ...
    {            
      ...
      {       
        if(count($g)>0)
        {
          ...
          foreach($g as $k=>$v)
          {          
            $s[] = array('title'=>'<table><tr><td style="padding-right: 10px;"><input class="' . $t . 'Filters' . $ft . '" name="' . $t . $k . '" id="' . $t . $k . '" value="' . $k . '" ' . (in_array($k,$selected)?'checked="checked"':'') . ' type="checkbox"></td><td>' . link_to($v, $path , array('query_string'=>'filter_by[' . $t .']=' . $k . ($params ? '&' . $params:''))) . '</td></table>'); 
          }
        }
      }
  ...
  public static function getFilterProjects($m,$path,$params=false,$selected=array(),$sf_user)
  {
    ...
    $s[] = array('title'=>'<form id="filter_by_' . $t .'_form" action="' . url_for($path . ($params ? '?' . $params:'')) . '" method="post">' . input_hidden_tag('filter_by[' . $t. ']',''). '<table onClick="filter_by_selected(\'' . $t . '\',\'' . $ft . '\')"><tr><td style="padding-right: 10px;">' . image_tag('icons/arrow_up.png') . '</td><td>' . __('Filter by selected') . '</td></table></form>','is_hr'=>true);
    $m[] = array('title'=>__('Cases'),'submenu'=>$s,'is_hr'=>true);
  ...
  public static function dueDateFormat($date,$is_listing = false)
  {
   ...
   if(strlen($date)>0)
   {
     ...
     {
       return ($is_listing?'<span class="hiddenItem">' . $date_sec . '</span>':'')  . str_replace(array('00:00','00:00:00'),'','<b>' . app::i18n_date(app::getDateTimeFormat(),$date_sec) . '</b>');
       
     }
     else
     {
      return ($is_listing?'<span class="hiddenItem">' . $date_sec . '</span>':'')  . str_replace(array('00:00','00:00:00'),'',app::i18n_date(app::getDateTimeFormat(),$date_sec));
     }
   } 
   else
   {
  ...
  public static function dateFormat($date,$timestamp=0,$is_listing = false)
  {
   if($timestamp>0)
   {
    return ($is_listing?'<span class="hiddenItem">' . $timestamp . '</span>':'')  . app::i18n_date(app::getDateFormat(),$timestamp);
   }
  ...
  public static function dateTimeFormat($date,$timestamp=0,$is_listing = false)
  {
   if($timestamp>0)
   {
     return ($is_listing?'<span class="hiddenItem">' . $timestamp . '</span>':'')  . str_replace(array('00:00','00:00:00'),'',app::i18n_date(app::getDateTimeFormat(),$timestamp));
   }
   elseif(strlen($date)>0)
   {
     $v = date_parse($date);
  ...
  public static function setCssForEmailContent($html)
  {
    ...
    $html = str_replace('<table class="contentTable">','<table style="' . sfConfig::get('app_email_style_table') . '">',$html);
